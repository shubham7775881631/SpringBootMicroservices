...
//	private RestTemplate restTemplate;
	...
	public ResponseEntity<RestEmployeeDTO> viewFullEmployeeProfile(@PathVariable Integer employeeId) throws EmployeeException {
		...
		List<ServiceInstance> instances = discoveryClient.getInstances("InfyAllocation");
		ServiceInstance instance = instances.get(0);
		URI infyAllocationUrl = instance.getUri();
		
		
		String allocationMsUrl = infyAllocationUrl+"/manager/getEmployeeAndManager/"+employeeId;
		
		//feteching the details from ms-Infyallocation
		
		RestEmployeeDTO restEmployeeDTO = new RestTemplate().getForObject(allocationMsUrl, RestEmployeeDTO.class);
		...
		List<ServiceInstance> instancesFin = discoveryClient.getInstances("Infy-EmployeeFinance");
		ServiceInstance finInst = instancesFin.get(0);
		URI finUrl = finInst.getUri();
		String financeMsUrl = finUrl+"/finance/getFinance/"+employeeDTO.getFinanceId();
		
		FinanceDTO financeDTO = new RestTemplate().getForObject(financeMsUrl,FinanceDTO.class);
