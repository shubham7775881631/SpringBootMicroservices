...
//	private RestTemplate restTemplate;
	...
	public Employee getEmployeeAndManager(@PathVariable Integer employeeId) throws ManagerException
	{
		...
		List<ServiceInstance> instances = discoveryClient.getInstances("EmployeeAndTrainingManagementSystemApplication");
		ServiceInstance instance =instances.get(0); 
		URI url = instance.getUri();
		
		String baseUrl=url+"/employee/getEmployee/"+employeeId;
		
		
		//by using the method getForObject(url,class_name.class)
		Employee employee = new RestTemplate().getForObject(baseUrl, Employee.class);
